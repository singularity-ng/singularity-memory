...
func clearModelDiscoveryEnv(t *testing.T) {
	...
	t.Setenv("SINGULARITY_MODEL_DISCOVERY_STORE_PATH", filepath.Join(t.TempDir(), "missing-model-discovery.json"))
}
...
func TestModelDiscoveryEndpointsFromStore(t *testing.T) {
	...
	t.Setenv("SINGULARITY_MODEL_DISCOVERY_STORE_PATH", storePath)
