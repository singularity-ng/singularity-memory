...
import (
	...
	"git.infra.centralcloud.com/centralcloud/operations-memory/internal/retrieve"
)
...
func (s *server) recall(w http.ResponseWriter, r *http.Request) {
	...
	// Semantic and temporal lanes require an embedding. Graph can expand from
	// semantic seeds when embeddings are present or BM25 seeds in text-only mode.
	...
	seedIDs := recallGraphSeedIDs(semanticResults, bm25Results, 10)
	if len(seedIDs) > 0 {
		graphStart := time.Now()
	...
	s.populateRecallResultEntities(ctx, bankID, results)
...
func recallGraphSeedIDs(primary []*retrieve.FullRetrievalResult, fallback []*retrieve.FullRetrievalResult, limit int) []string {
	source := primary
	if len(source) == 0 {
		source = fallback
	}
	...
	ids := make([]string, 0, min(len(source), limit))
...
func (s *server) populateRecallResultEntities(ctx context.Context, bankID string, results []recallResult) {
	if len(results) == 0 || s.deps.Store == nil {
		return
	}
	...
	query := `
		...
		WHERE e.bank_id = $1
		  AND ue.unit_id = ANY($2::uuid[])
		ORDER BY e.canonical_name
	`
	rows, err := s.deps.Store.Query(ctx, query, bankID, ids)
	if err != nil {
		s.logRecallError("load recall entities failed", err)
		return
	}
	if rows == nil {
		return
	...
	seen := make(map[string]map[string]struct{}, len(results))
	for rows.Next() {
		var unitID, name string
		if err := rows.Scan(&unitID, &name); err != nil {
			s.logRecallError("scan recall entity failed", err)
			return
		}
		i, ok := index[unitID]
		if !ok {
		...
		if seen[unitID] == nil {
			seen[unitID] = make(map[string]struct{})
		}
		if _, ok := seen[unitID][name]; ok {
			continue
		}
		seen[unitID][name] = struct{}{}
		results[i].Entities = append(results[i].Entities, name)
	}
	if err := rows.Err(); err != nil {
		s.logRecallError("load recall entities rows failed", err)
	}
}
