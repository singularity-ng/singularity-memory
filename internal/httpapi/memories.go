...
func (s *server) retain(w http.ResponseWriter, r *http.Request) {
	...
	// Two-level batching: split items by OPS_MEMORY_RETAIN_BATCH_TOKENS
...
func (s *server) storeUnit(ctx context.Context, bankID string, unit *processedMemoryUnit) error {
	...
	if err := s.storeEntityMentions(ctx, bankID, unitID, unit.Entities); err != nil {
		return err
	}
	return nil
}
...
func (s *server) storeEntityMentions(ctx context.Context, bankID string, unitID string, entities []string) error {
	if len(entities) == 0 {
		return nil
	}
	...
	for _, entityName := range entities {
		...
		entityID, err := s.deps.Store.UpsertEntity(ctx, bankID, entityName)
		if err != nil {
			return fmt.Errorf("upsert entity %q: %w", entityName, err)
		}
		if err := s.deps.Store.LinkUnitEntity(ctx, unitID, entityID); err != nil {
			return fmt.Errorf("link entity %q: %w", entityName, err)
		}

		neighborIDs, err := s.deps.Store.RecentEntityUnitIDs(ctx, bankID, entityID, unitID, 5)
		if err != nil {
			return fmt.Errorf("load entity neighbors %q: %w", entityName, err)
		}
		for _, neighborID := range neighborIDs {
			link := &store.MemoryLink{
				FromUnitID: neighborID,
				ToUnitID:   unitID,
				LinkType:   "entity",
				EntityID:   &entityID,
				BankID:     bankID,
			...
			if err := s.deps.Store.InsertMemoryLink(ctx, link); err != nil {
				return fmt.Errorf("insert entity link %q: %w", entityName, err)
			}
		}
	}
	return nil
