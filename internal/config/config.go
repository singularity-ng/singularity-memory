...
const (
	...
	defaultRerankModel             = "qwen/qwen3-reranker-0.6b"
	defaultRerankTopK              = 10
	defaultStorageProfile          = "vchord"
	defaultModelCatalogPath        = ".operations-memory/model-catalog.json"
	defaultModelDiscoveryStorePath = ".operations-memory/model-discovery.json"
...
func FromEnv() Config {
	profile, _ := storageprofile.ParseProfile(getenvAny(defaultStorageProfile, "OPS_MEMORY_STORAGE_PROFILE"))
	modelDiscoveryStorePath := getenvAny(defaultModelDiscoveryStorePath, "OPS_MEMORY_MODEL_DISCOVERY_STORE_PATH")
	...
	if secretSource == "" {
		secretSource = getenvAny(defaultSecretSource, "OPS_MEMORY_MODEL_DISCOVERY_SECRET_SOURCE")
	}
	...
	if len(definitions) == 0 {
		definitions = parseModelDiscoveryEndpointSpecs(getenvAny("", "OPS_MEMORY_MODEL_DISCOVERY_ENDPOINTS"))
	}
	...
	return Config{
		...
		ModelCatalogPath:           getenvAny(defaultModelCatalogPath, "OPS_MEMORY_MODEL_CATALOG_PATH"),
		...
		RRFWeights:        parseRRFWeights(getenvAny(defaultRRFWeights, "OPS_MEMORY_RRF_WEIGHTS")),
	}
}
...
func loadModelDiscoverySecrets(secretSource string) (modelDiscoverySecretBundle, error) {
	...
	case "sf-sops", "sf-sops,env", "sf-sops+env":
		return loadSFSOPSSecrets(
			getenvAny(defaultSOPSSecretsPath, "OPS_MEMORY_MODEL_DISCOVERY_SOPS_FILE"),
			getenvAny(defaultSOPSConfigPath, "OPS_MEMORY_MODEL_DISCOVERY_SOPS_CONFIG"),
		)
	default:
