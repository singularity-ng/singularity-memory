...
func testDB(t *testing.T) *Store {
	...
	if dsn == "" {
		dsn = os.Getenv("OPS_MEMORY_DATABASE_URL")
	}
	if dsn == "" {
		t.Skip("TEST_DATABASE_URL/OPS_MEMORY_DATABASE_URL is not set")
	}
...
func requireIntegrationPrereqs(t *testing.T, ctx context.Context, db *Store) {
	...
	for _, table := range requiredTables {
		...
		if err != nil || !exists {
			db.Close()
			t.Skipf("required table %s is missing; run the Operations Memory schema migrations first", regclass)
		}
	}
}
