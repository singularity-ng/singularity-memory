...
// EnsureBrainSchema installs the agent-facing memory surfaces inside the same
// database. Memory rows remain the retrieval substrate; these tables preserve
// always-in-context blocks plus page/source/link/timeline/job semantics.
func (s *Store) EnsureBrainSchema(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS ` + s.table("core_memory_blocks") + ` (
			bank_id text NOT NULL REFERENCES ` + s.table("banks") + ` (bank_id) ON DELETE CASCADE,
			block_name text NOT NULL,
			content text NOT NULL DEFAULT '',
			char_limit integer NOT NULL DEFAULT 2000 CHECK (char_limit > 0),
			description text NOT NULL DEFAULT '',
			created_at timestamptz NOT NULL DEFAULT now(),
			updated_at timestamptz NOT NULL DEFAULT now(),
			PRIMARY KEY (bank_id, block_name)
		)`,
		`CREATE INDEX IF NOT EXISTS idx_core_memory_blocks_bank ON ` + s.table("core_memory_blocks") + ` (bank_id)`,
		`CREATE TABLE IF NOT EXISTS ` + s.table("consolidation_state") + ` (
			bank_id text PRIMARY KEY REFERENCES ` + s.table("banks") + ` (bank_id) ON DELETE CASCADE,
