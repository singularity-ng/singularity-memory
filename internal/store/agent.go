...
func (s *Store) AppendCoreMemoryBlock(ctx context.Context, bankID string, blockName string, text string) (*CoreMemoryBlock, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT content, char_limit, description
		FROM `+s.table("core_memory_blocks")+`
		WHERE bank_id = $1 AND block_name = $2
	`, bankID, blockName).Scan(&content, &limit, &description)
	if err != nil {
		if err != pgx.ErrNoRows {
			return nil, fmt.Errorf("load core memory block: %w", err)
		}
		limit = 2000
	}
	...
	next += text
	truncated := len(next) > limit
	if truncated {
		next = next[:limit]
	}

	block, err := s.UpsertCoreMemoryBlock(ctx, CoreMemoryBlock{
		BankID:      bankID,
		BlockName:   blockName,
		Content:     next,
		CharLimit:   limit,
...
func (s *Store) ReplaceCoreMemoryBlock(ctx context.Context, bankID string, blockName string, oldText string, newText string) (*CoreMemoryBlock, error) {
	bankID = strings.TrimSpace(bankID)
	blockName = strings.TrimSpace(blockName)
	if bankID == "" {
		return nil, fmt.Errorf("bank_id is required")
	...
	err := s.pool.QueryRow(ctx, `
		...
		FROM `+s.table("core_memory_blocks")+`
		WHERE bank_id = $1 AND block_name = $2
	`, bankID, blockName).Scan(
		&block.BankID, &block.BlockName, &block.Content, &block.CharLimit, &block.Description, &block.CreatedAt, &block.UpdatedAt,
	)
	if err != nil {
		if err == pgx.ErrNoRows {
			return nil, fmt.Errorf("core memory block %q not found in bank %q", blockName, bankID)
		}
		return nil, fmt.Errorf("load core memory block: %w", err)
	}
	if oldText == "" {
	...
	if !strings.Contains(block.Content, oldText) {
		return nil, fmt.Errorf("old_text not found in block %q", blockName)
	}

	replaced := strings.Replace(block.Content, oldText, newText, 1)
	truncated := len(replaced) > block.CharLimit
	if truncated {
		replaced = replaced[:block.CharLimit]
	}

	err = s.pool.QueryRow(ctx, `
		UPDATE `+s.table("core_memory_blocks")+`
		SET content = $1, updated_at = now()
		WHERE bank_id = $2 AND block_name = $3
		RETURNING bank_id, block_name, content, char_limit, description, created_at, updated_at
	`, replaced, bankID, blockName).Scan(
		&block.BankID, &block.BlockName, &block.Content, &block.CharLimit, &block.Description, &block.CreatedAt, &block.UpdatedAt,
	)
	if err != nil {
		return nil, fmt.Errorf("replace core memory block: %w", err)
	}
	block.Truncated = truncated
	return &block, nil
}

func (s *Store) DeleteCoreMemoryBlock(ctx context.Context, bankID string, blockName string) (bool, error) {
	tag, err := s.pool.Exec(ctx, `
		DELETE FROM `+s.table("core_memory_blocks")+`
		WHERE bank_id = $1 AND block_name = $2
	`, strings.TrimSpace(bankID), strings.TrimSpace(blockName))
	if err != nil {
		return false, fmt.Errorf("delete core memory block: %w", err)
	}
