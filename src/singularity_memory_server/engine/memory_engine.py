...
if TYPE_CHECKING:
    from singularity_memory_server.extensions import OperationValidatorExtension, TenantExtension
...
class MemoryEngine(MemoryEngineInterface):
    ...
    def __init__(
        ...
        See singularity_memory_server.config for environment variable names and defaults.

        Args:
            db_url: PostgreSQL connection URL. Defaults to SINGULARITY_DATABASE_URL env var or "pg0".
                    Also supports pg0 URLs: "pg0" or "pg0://instance-name" or "pg0://instance-name:port"
            memory_llm_provider: LLM provider. Defaults to SINGULARITY_LLM_PROVIDER env var or "groq".
            memory_llm_api_key: API key for the LLM provider. Defaults to SINGULARITY_LLM_API_KEY env var.
            memory_llm_model: Model name. Defaults to SINGULARITY_LLM_MODEL env var.
            ...
            pool_min_size: Minimum number of connections in the pool. Defaults to SINGULARITY_DB_POOL_MIN_SIZE.
            pool_max_size: Maximum number of connections in the pool. Defaults to SINGULARITY_DB_POOL_MAX_SIZE.
            db_command_timeout: PostgreSQL command timeout in seconds. Defaults to SINGULARITY_DB_COMMAND_TIMEOUT.
            db_acquire_timeout: Connection acquisition timeout in seconds. Defaults to SINGULARITY_DB_ACQUIRE_TIMEOUT.
            ...
            skip_llm_verification: Skip LLM connection verification during initialization.
                                  Defaults to SINGULARITY_SKIP_LLM_VERIFICATION env var or False.
            lazy_reranker: Delay reranker initialization until first use. Useful for retain-only
                          ...
                          SINGULARITY_LAZY_RERANKER env var or False.
        """
        ...
        if not memory_llm_api_key and requires_api_key(memory_llm_provider):
            raise ValueError("LLM API key is required. Set SINGULARITY_LLM_API_KEY environment variable.")
        ...
        # Configurable via SINGULARITY_RECALL_MAX_CONCURRENT (default: 50)
        ...
        # Configurable via SINGULARITY_RETAIN_MAX_CONCURRENT (default: 4).
    ...
    async def _validate_operation(self, validation_coro) -> "ValidationResult | None":
        ...
        from singularity_memory_server.extensions import OperationValidationError, ValidationResult
    ...
    async def _authenticate_tenant(self, request_context: "RequestContext | None") -> str:
        ...
        """
        from singularity_memory_server.extensions import AuthenticationError
    ...
    async def _handle_file_convert_retain(self, task_dict: dict[str, Any]):
        ...
        if self._operation_validator:
            try:
                from singularity_memory_server.extensions.operation_validator import FileConvertResult
                from singularity_memory_server.models import RequestContext
    ...
    async def _handle_refresh_mental_model(self, task_dict: dict[str, Any]):
        ...
        if self._operation_validator:
            from singularity_memory_server.extensions.operation_validator import MentalModelRefreshResult
    ...
    async def _handle_webhook_delivery(self, task_dict: dict[str, Any]) -> None:
        ...
        headers: dict[str, str] = {
            ...
            "X-Singularity-Memory-Event": event_type,
            **http_config.headers,
        }
        if secret and self._webhook_manager:
            headers["X-Singularity-Memory-Signature"] = self._webhook_manager._sign_payload(secret, payload_bytes)
    ...
    def retain(
        ...
        from singularity_memory_server.models import RequestContext as RC
    ...
    async def retain_batch_async(
        ...
        *,
        ...
        if self._operation_validator:
            from singularity_memory_server.extensions import RetainContext
        ...
        # Call post-operation hook if validator is configured
        if self._operation_validator:
            from singularity_memory_server.extensions import RetainResult
    ...
    async def _retain_batch_async_internal(
        ...
        from singularity_memory_server.config_resolver import apply_strategy
    ...
    async def recall_async(
        ...
        *,
        ...
        # Validate operation if validator is configured
        if self._operation_validator:
            from singularity_memory_server.extensions import RecallContext
        ...
        recall_span_context = tracer.start_as_current_span("singularity_memory.recall")
        recall_span = recall_span_context.__enter__()
        recall_span.set_attribute("singularity_memory.bank_id", bank_id)
        recall_span.set_attribute("singularity_memory.query", query[:100])
        recall_span.set_attribute("singularity_memory.fact_types", ",".join(fact_type))
        recall_span.set_attribute("singularity_memory.thinking_budget", thinking_budget)
        recall_span.set_attribute("singularity_memory.max_tokens", max_tokens)
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        if is_connection_error and attempt < max_retries:
                            ...
                            error_msg = str(e)
                            if self._operation_validator:
                                from singularity_memory_server.extensions.operation_validator import RecallResult
    ...
    async def _search_with_retries(
        ...
        try:
            ...
            retrieval_span = tracer_otel.start_span("singularity_memory.recall_retrieval")
            retrieval_span.set_attribute("singularity_memory.bank_id", bank_id)
            retrieval_span.set_attribute("singularity_memory.fact_types", ",".join(fact_type))
            retrieval_span.set_attribute("singularity_memory.thinking_budget", thinking_budget)
            ...
            fusion_span = tracer_otel.start_span("singularity_memory.recall_fusion")
            fusion_span.set_attribute("singularity_memory.bank_id", bank_id)
            fusion_span.set_attribute("singularity_memory.semantic_count", len(semantic_results))
            fusion_span.set_attribute("singularity_memory.bm25_count", len(bm25_results))
            fusion_span.set_attribute("singularity_memory.graph_count", len(graph_results))
            ...
            try:
                ...
                fusion_span.set_attribute("singularity_memory.merged_count", len(merged_candidates))
                fusion_span.end()
            ...
            rerank_span = tracer_otel.start_span("singularity_memory.recall_rerank")
            rerank_span.set_attribute("singularity_memory.bank_id", bank_id)
            rerank_span.set_attribute("singularity_memory.candidates_count", len(merged_candidates))
            ...
            try:
                ...
                rerank_span.set_attribute("singularity_memory.scored_count", len(scored_results))
                if pre_filtered_count > 0:
                    rerank_span.set_attribute("singularity_memory.pre_filtered_count", pre_filtered_count)
                rerank_span.end()
    ...
    async def get_document(
        ...
        *,
        ...
        await self._authenticate_tenant(request_context)
        if self._operation_validator:
            from singularity_memory_server.extensions import BankReadContext
    ...
    async def delete_document(
        ...
        *,
        ...
        await self._authenticate_tenant(request_context)
        if self._operation_validator:
            from singularity_memory_server.extensions import BankWriteContext
    ...
    async def get_bank_profile(
        ...
        *,
        ...
        # Apply SINGULARITY_DEFAULT_BANK_TEMPLATE to freshly-created banks. Done
    ...
    async def _apply_default_bank_template(
        ...
        """Apply SINGULARITY_DEFAULT_BANK_TEMPLATE to a freshly-created bank.
        ...
        from singularity_memory_server.api.http import (
        ...
        try:
            ...
            logger.error(
                "SINGULARITY_DEFAULT_BANK_TEMPLATE failed schema validation "
        ...
        if semantic_errors:
            logger.error(
                "SINGULARITY_DEFAULT_BANK_TEMPLATE failed semantic validation "
        ...
        try:
            ...
            logger.info(f"Applied SINGULARITY_DEFAULT_BANK_TEMPLATE to newly-created bank '{bank_id}'")
        except Exception as e:
            logger.error(f"Failed to apply SINGULARITY_DEFAULT_BANK_TEMPLATE to bank '{bank_id}': {e}")
    ...
    async def list_banks(
        ...
        *,
        ...
        if self._operation_validator:
            from singularity_memory_server.extensions import BankListContext
    ...
    async def reflect_async(
        ...
        *,
        ...
        if self._reflect_llm_config is None:
            raise ValueError("Memory LLM API key not set. Set SINGULARITY_LLM_API_KEY environment variable.")
        ...
        if self._llm_config.provider == "none":
            ...
            raise LLMNotAvailableError(
                ...
                "Set SINGULARITY_LLM_PROVIDER to a real provider (e.g., openai, anthropic, gemini)."
            )
        ...
        # Validate operation if validator is configured
        if self._operation_validator:
            from singularity_memory_server.extensions import ReflectContext
        ...
        try:
            ...
            from singularity_memory_server.engine.response_models import DirectiveRef
            ...
            from singularity_memory_server.engine.response_models import TokenUsage
            ...
            # Call post-operation hook if validator is configured
            if self._operation_validator:
                from singularity_memory_server.extensions.operation_validator import ReflectResultContext
    ...
    async def get_mental_model(
        ...
        *,
        ...
        if self._operation_validator:
            from singularity_memory_server.extensions.operation_validator import MentalModelGetContext
        ...
        if result and self._operation_validator:
            from singularity_memory_server.extensions.operation_validator import MentalModelGetResult
    ...
    async def refresh_mental_model(
        ...
        *,
        ...
        with create_operation_span("mental_model_refresh", bank_id):
            ...
            # Skip creating a nested "singularity_memory.reflect" span since we already have "singularity_memory.mental_model_refresh"
    ...
    async def compute_mental_model_is_stale(
        ...
        """
        from singularity_memory_server.engine.search.tags import _parse_tags_match
    ...
    async def retry_operation(
        ...
        *,
        ...
        await self._authenticate_tenant(request_context)
        from singularity_memory_server.extensions import OperationValidationError
    ...
    async def submit_async_refresh_mental_model(
        ...
        *,
        ...
        if self._operation_validator:
            from singularity_memory_server.extensions.operation_validator import MentalModelRefreshContext
