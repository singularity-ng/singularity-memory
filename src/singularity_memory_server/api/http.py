...
from singularity_memory_server.engine.audit import AuditEntry, AuditLogger
...
from singularity_memory_server.engine.memory_engine import Budget, _current_schema, _get_tiktoken_encoding, fq_table
from singularity_memory_server.engine.providers.none_llm import LLMNotAvailableError
from singularity_memory_server.engine.response_models import VALID_RECALL_FACT_TYPES, MemoryFact, TokenUsage
from singularity_memory_server.engine.search.tags import TagGroup, TagsMatch
from singularity_memory_server.extensions import HttpExtension, OperationValidationError, load_extension
from singularity_memory_server.metrics import create_metrics_collector, get_metrics_collector, initialize_metrics
...
class BankConfigUpdate(BaseModel):
    ...
    updates: dict[str, Any] = Field(
        ...
        "or environment variable format (SINGULARITY_LLM_PROVIDER). "
    ...
    def validate_version(cls, v: str) -> str:
        ...
        if ver > int(BANK_TEMPLATE_CURRENT_VERSION):
            raise ValueError(
                ...
                f"(max supported: {BANK_TEMPLATE_CURRENT_VERSION}). Please upgrade Singularity Memory."
            )
        return v
...
async def apply_bank_template_manifest(
    ...
    driven by SINGULARITY_DEFAULT_BANK_TEMPLATE. The bank MUST already
...
from singularity_memory_server.webhooks.models import WebhookHttpConfig
...
def create_app(
    ...
    Args:
        ...
        http_extension: Optional HTTP extension to mount custom endpoints under /extension/.
                       If None, attempts to load from SINGULARITY_HTTP_EXTENSION env var.
    ...
    async def lifespan(app: FastAPI):
        ...
        from singularity_memory_server.config import get_config
        from singularity_memory_server.worker import WorkerPoller
        ...
        try:
            prometheus_reader = initialize_metrics(service_name="singularity-memory", service_version="1.0.0")
        ...
        if config.otel_traces_enabled:
            if not config.otel_exporter_otlp_endpoint:
                ...
                from singularity_memory_server.tracing import create_span_recorder, initialize_tracing
    ...
    from singularity_memory_server import __version__
    from singularity_memory_server.config import get_config

    config = get_config()

    app = FastAPI(
        title="Singularity Memory HTTP API",
        version=__version__,
        description="HTTP API for Singularity Memory",
        contact={
            "name": "Memory System",
        },
        license_info={
...
def _register_routes(app: FastAPI):
    ...
    async def version_endpoint() -> VersionResponse:
        ...
        """
        from singularity_memory_server import __version__
        from singularity_memory_server.config import _get_raw_config
    ...
    async def api_get_bank_config(bank_id: str, request_context: RequestContext = Depends(get_request_context)):
        ...
        if not get_config().enable_bank_config_api:
            raise HTTPException(
                status_code=404,
                detail="Bank configuration API is disabled. Set SINGULARITY_ENABLE_BANK_CONFIG_API=true to re-enable.",
            )
        try:
    ...
    @app.patch(
        ...
        "Keys can be provided in Python field format (llm_provider) or environment variable format (SINGULARITY_LLM_PROVIDER).",
    ...
    async def api_create_webhook(
        ...
        try:
            ...
            from singularity_memory_server.engine.memory_engine import fq_table
            from singularity_memory_server.engine.retain import bank_utils
            ...
            if request.async_:
                ...
                if config.retain_batch_enabled:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            "Batch API is enabled (SINGULARITY_RETAIN_BATCH_ENABLED=true) but async=false. "
                            ...
                            "by setting SINGULARITY_RETAIN_BATCH_ENABLED=false in your environment."
                        ),
                    )
    ...
    @app.post(
        ...
        "- Falls back to the server default (`SINGULARITY_FILE_PARSER`) if not specified.\n"
    ...
    async def api_file_retain(
        ...
        if not config.enable_file_upload_api:
            raise HTTPException(
                status_code=404,
                detail="File upload API is disabled. Set SINGULARITY_ENABLE_FILE_UPLOAD_API=true to enable.",
            )
    ...
    async def api_list_audit_logs(
        ...
        try:
            ...
            from singularity_memory_server.engine.db_utils import acquire_with_retry
